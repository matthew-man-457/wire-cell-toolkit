...
RegionOfInterestFilter::RegionOfInterestFilter(const std::string& roi_tag, const std::string& old_tag, float peak, int ROI_t, int ROI_ch)
: m_roi_tag(roi_tag)
, m_old_tag(old_tag)
, m_frame_tag("sigproc")
, m_peak(peak)
...
void RegionOfInterestFilter::configure(const WireCell::Configuration& cfg)
{
  ...
  m_peak = get(cfg,"peak",m_peak);
  m_ROI_t = get(cfg,"ROI_t",m_ROI_t);
  m_ROI_ch = get(cfg,"ROI_ch",m_ROI_ch);
}
...
bool RegionOfInterestFilter::operator()(const input_pointer& inframe, output_pointer& outframe)
{
  ...
  //std::cout << "peak: " << m_peak << ", ROI_t: " << m_ROI_t << ", ROI_ch: " << m_ROI_ch << std::endl;
  ...
  TFile bkgFile("/dune/app/users/mman/wct_roi_man/rootfiles/background.root", "READ"); // <--
  ...
  for (auto trace : *traces.get())
  {
    ...
    for (int bin = 0; bin < (int)charges.size(); ++bin)
    {
      ...
      if(upper_time_roi_counter > 0){
        ...
        newcharge.at(bin) = 0.0; // <---
      }

      if(central_value<-m_peak or central_value>m_peak)
      {
